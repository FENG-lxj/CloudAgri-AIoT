...
func (u *userToken) GenerateToken(phone string, expireAt int64) (tokens string, err error) {
...
func (u *userToken) RecordLoginToken(phone, created_at, userToken, clientIp string) bool {
	if customClaims, err := u.userJwt.ParseToken(userToken); err == nil {
		expiresAt := customClaims.ExpiresAt
		return model.CreateUserFactory("").OauthLoginToken(phone, userToken, created_at, expiresAt, clientIp)
	} else {
		return false
	}
}
...
func (u *userToken) TokenIsMeetRefreshCondition(token string) bool {
	...
	case consts.JwtTokenOK, consts.JwtTokenExpired:
		...
		if model.CreateUserFactory("").OauthRefreshConditionCheck(customClaims.Phone, token) {
			return true
		}
	}
	return false
...
func (u *userToken) RefreshToken(oldToken, clientIp, ipAddr string) (newToken string, res bool) {
	var err error
	...
	if newToken, err = u.userJwt.RefreshToken(oldToken, variable.ConfigYml.GetInt64("Token.JwtTokenRefreshExpireAt")); err == nil {
		if customClaims, err := u.userJwt.ParseToken(newToken); err == nil {
			...
			expiresAt := customClaims.ExpiresAt
			if model.CreateUserFactory("").OauthRefreshToken(expiresAt, phone, oldToken, newToken, clientIp, ipAddr) {
...
func (u *userToken) IsEffective(token string) bool {
	...
	if consts.JwtTokenOK == code {
		...
		if variable.ConfigYml.GetInt("Token.IsCacheToRedis") == 1 {
			tokenRedisFact := token_cache_redis.CreateUsersTokenCacheFactory(customClaims.Phone)
		...
		if model.CreateUserFactory("").OauthCheckTokenIsOk(customClaims.Phone, token) {
			return true
		}
	}
	return false
